...
function makeService(db: {
  ...
  const tx = {
    fleet: {
      create: vi.fn(
        async (args: { data: { name: string; ownerId: string } }) => ({
          id: 'fleet-new',
          ...args.data,
        }),
      ),
  ...
  const system = {
    ...
    role: {
      findUnique: vi.fn(async () =>
        'role' in db
          ? db.role
          : { id: 'role-ind', slug: 'independent_driver', isActive: true },
      ),
    },
  ...
  const service = new FleetsService(
    system as never,
    {} as never,
    audit as never,
  );
...
describe('FleetsService.ensurePersonalFleet', () => {
  ...
  it('provisions fleet + ACTIVE independent_driver membership and audits', async () => {
    ...
    expect(tx.fleet.create).toHaveBeenCalledWith({
      data: { name: "Sami's Fleet", ownerId: 'driver-1' },
    });
    ...
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'driver.fleet.provision' }),
    );
  });
  ...
  it('fails closed when the independent_driver role is not seeded', async () => {
    ...
    await expect(
      service.ensurePersonalFleet('driver-1', 'Sami'),
    ).rejects.toThrow();
    expect(tx.fleet.create).not.toHaveBeenCalled();
  });
});
