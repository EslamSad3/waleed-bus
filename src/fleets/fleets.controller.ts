...
import { CreateFleetDto, FleetDto, MyMembershipDto, UpdateFleetDto } from '../users/dto/user.dto.js';
...
export class FleetsController {
  ...
  @ApiOperation({ summary: 'Create a fleet (platform path; optionally grants the owner an initial membership).' })
  @ApiEnvelopeResponse(201, 'Fleet created.', FleetDto)
  @ApiConflict('Owner role slug unknown/inactive, or the owner user cannot be linked.')
  ...
  @ApiOperation({ summary: 'List all fleets (platform path, cursor pagination).' })
  @ApiCursorPagination()
  @ApiEnvelopeResponse(200, 'Cursor page of fleets (items + nextCursor).', FleetDto, true)
  findAll(@Query() query: { cursor?: string; limit?: string }) {
    return this.fleetsService.findAll(query);
  }

  @Get('mine')
  @ApiOperation({ summary: 'List the caller\'s own ACTIVE fleet memberships (RLS tenant path).' })
  @ApiEnvelopeResponse(200, 'The caller\'s ACTIVE memberships — never another user\'s.', MyMembershipDto, true)
  ...
  @ApiNotFound('Fleet not found.')
  update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateFleetDto, @CurrentUser() actor: RequestUser) {
  ...
  @ApiOperation({ summary: 'Delete a fleet and its memberships (platform path, cascading).' })
  @ApiUuidParam('id', 'Fleet id (uuid).')
