...
function makeTx(db: FakeDb) {
  return {
    ...
    busAssignment: {
      ...
      create: vi.fn(async (args: { data: unknown }) => ({ id: 'assign-new', ...(args.data as object) })),
    },
    booking: {
...
function makeService(db: FakeDb) {
  ...
  const tenantContext = {
    withFleetContext: vi.fn(async (_i: unknown, fn: (tx: unknown) => Promise<unknown>) => fn(tx)),
  };
  const audit = { log: vi.fn(async () => undefined) };
  const system = { user: { findUnique: vi.fn(async () => null) } };
  const service = new DriverOpsService(fleetPath as never, tenantContext as never, system as never, audit as never);
...
function execContext(params: Record<string, string>, extra: Record<string, unknown> = {}) {
...
describe('DriverTripGuard', () => {
  ...
  it('rejects cross-fleet or unassigned trips with 404 TRIP_ACCESS_DENIED', async () => {
    ...
    service.assertAssignment.mockRejectedValue(
      new CodedException(404, 'TRIP_ACCESS_DENIED', 'Trip not found on the assigned bus.'),
    );
    const error = await guard.canActivate(execContext({ tripId: 'trip-x' })).catch((e: unknown) => e);
  ...
  it('rejects requests without a verified fleet context (deny by default)', async () => {
    ...
    const ctx = execContext({ tripId: 'trip-1' }, { fleetContext: undefined, user: undefined });
...
describe('DriverOpsService.assertAssignment', () => {
  it('anchors on the in-tx trip row and requires a matching ACTIVE assignment', async () => {
    const { service, tx } = makeService({
      trip: { id: 'trip-1', fleetId: 'fleet-1', busId: 'bus-1', status: 'DEPARTED' },
  ...
  it('maps a foreign trip to 404 TRIP_ACCESS_DENIED (never 403)', async () => {
    ...
    const error = await service
      .assertAssignment(tx as never, { driverId: 'driver-1', fleetId: 'fleet-1', tripId: 'nope' })
      .catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect((error as CodedException).getStatus()).toBe(404);
    expect(JSON.stringify((error as CodedException).getResponse())).toContain('TRIP_ACCESS_DENIED');
  });
  ...
  it('rejects booking ids that do not belong to the path trip (404 BOOKING_NOT_ON_TRIP)', async () => {
    ...
    const error = await service
      .assertAssignment(tx as never, { driverId: 'driver-1', fleetId: 'fleet-1', tripId: 'trip-1', bookingId: 'b-1' })
      .catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect((error as CodedException).getStatus()).toBe(404);
  });
...
describe('DriverOpsService.claimBus', () => {
  ...
  it('lets the fleet owner claim an owned bus (independent-driver path)', async () => {
    ...
    expect(result).toMatchObject({ driverUserId: 'driver-1', busId: 'bus-1', status: 'ACTIVE' });
  });

  it('rejects claims by non-owners with 403 (fleet drivers cannot self-assign)', async () => {
    const { service, tx } = makeService({ ...owned, fleet: { id: 'fleet-1', ownerId: 'someone-else' } });
    const error = await service.claimBus(ACTOR, FLEET, 'bus-1').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
  ...
  it('maps foreign buses to 404 (no oracle)', async () => {
    ...
    const error = await service.claimBus(ACTOR, FLEET, 'foreign-bus').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect((error as CodedException).getStatus()).toBe(404);
  });
});
...
describe('DriverOpsService transitions', () => {
  it('board repeats converge (already BOARDED → 200 same, no write)', async () => {
    const booking = { id: 'b-1', tripId: 'trip-1', status: 'CONFIRMED', boardedAt: new Date('2026-09-11T10:00:00Z') };
    const { service, tx } = makeService({
      trip: { id: 'trip-1', fleetId: 'fleet-1', busId: 'bus-1', status: 'DEPARTED' },
      assignment: { id: 'a-1', status: 'ACTIVE' },
      booking,
  ...
  it('board rejects cancelled bookings with 409', async () => {
    const { service } = makeService({
      trip: { id: 'trip-1', fleetId: 'fleet-1', busId: 'bus-1', status: 'DEPARTED' },
      assignment: { id: 'a-1', status: 'ACTIVE' },
      booking: { id: 'b-1', tripId: 'trip-1', status: 'CANCELLED', boardedAt: null },
    });
    const error = await service.board(ACTOR, FLEET, 'trip-1', 'b-1').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
  ...
  it('drop-off requires boarding; conflicting re-drop conflicts with 409 INVALID_DROPOFF_STATE', async () => {
    ...
    const unboarded = makeService({ ...base, booking: { id: 'b-1', tripId: 'trip-1', status: 'CONFIRMED', boardedAt: null } });
    const err1 = await unboarded.service
      .dropOff(ACTOR, FLEET, 'trip-1', 'b-1', { status: 'DROPPED_OFF', stationId: 'S1' })
      .catch((e: unknown) => e);
    expect((err1 as CodedException).getStatus()).toBe(409);

    const dropped = makeService({
      ...base,
      booking: { id: 'b-1', tripId: 'trip-1', status: 'CONFIRMED', boardedAt: new Date(), dropStatus: 'DROPPED_OFF' },
    });
    const err2 = await dropped.service
      .dropOff(ACTOR, FLEET, 'trip-1', 'b-1', { status: 'NOT_DROPPED_OFF', reason: 'changed mind' })
      .catch((e: unknown) => e);
    expect(err2).toBeInstanceOf(CodedException);
    expect(JSON.stringify((err2 as CodedException).getResponse())).toContain('INVALID_DROPOFF_STATE');
  });
  ...
  it('cash payment requires boarded + unpaid; repeat PAID converges', async () => {
    ...
    const paid = makeService({
      ...base,
      booking: { id: 'b-1', tripId: 'trip-1', status: 'CONFIRMED', boardedAt: new Date(), paymentStatus: 'PAID' },
    });
    const result = await paid.service.cashPayment(ACTOR, FLEET, 'trip-1', 'b-1', { method: 'CASH', status: 'PAID' });
    ...
    const methodMismatch = makeService({
      ...base,
      booking: { id: 'b-1', tripId: 'trip-1', status: 'CONFIRMED', boardedAt: new Date(), paymentMethod: 'CARD', paymentStatus: null },
    });
    const error = await methodMismatch.service
      .cashPayment(ACTOR, FLEET, 'trip-1', 'b-1', { method: 'CASH', status: 'PAID' })
      .catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect(JSON.stringify((error as CodedException).getResponse())).toContain('PAYMENT_NOT_ALLOWED');
  });
});
