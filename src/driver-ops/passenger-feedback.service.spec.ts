...
function makeService(db: FakeDb) {
  const tx = {
    ...
    passengerReport: {
      create: vi.fn(async () => db.created ?? { id: 'rep-1' }),
    },
  };
  const anchored = {
    trip: {
      ...
      fleetId: 'fleet-1',
      busId: 'bus-1',
      status: db.tripStatus ?? 'COMPLETED',
    },
  ...
  const service = new PassengerFeedbackService(
    fleetPath as never,
    driverOps as never,
    audit as never,
  );
...
describe('PassengerFeedbackService', () => {
  it('driver rating requires a COMPLETED trip (409 RATING_NOT_ALLOWED otherwise)', async () => {
    ...
    const error = await service
      .ratePassenger(ACTOR, FLEET, 'trip-1', 'b-1', 5)
      .catch((e: unknown) => e);
  ...
  it('driver rating is single-write-per-side: repeat same → 200, change → 409', async () => {
    ...
    const ok = await same.service.ratePassenger(
      ACTOR,
      FLEET,
      'trip-1',
      'b-1',
    ...
    const error = await changed.service
      .ratePassenger(ACTOR, FLEET, 'trip-1', 'b-1', 3)
      .catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
  ...
  it('report appends a passenger_reports row and audits', async () => {
    ...
    const result = await service.reportPassenger(
      ACTOR,
      FLEET,
      'trip-1',
      'b-1',
    ...
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'driver.passenger.report' }),
    );
  });
});
