...
export class DriverOpsService {
    ...
    const run = async (tx: Prisma.TransactionClient) => {
      ...
      if (!assignment) {
        throw new CodedException(
          404,
          'DRIVER_NOT_ASSIGNED',
          'No active bus assignment for this driver.',
        );
      }
      const bus =
        fleetContext.membershipId === null
          ? await tx.bus.findFirst({
              where: { id: assignment.busId, fleetId: fleetContext.fleetId },
            })
    ...
    const run = async (
      ...
      if (!fleet || fleet.ownerId !== actor.id) {
        throw new CodedException(
          403,
          'FORBIDDEN',
          'Only the fleet owner may claim a bus.',
        );
      }
      ...
      if (!membership) {
        throw new CodedException(
          ...
          'No ACTIVE driver membership in this fleet.',
        );
      }
    ...
    const run = async (tx: Prisma.TransactionClient) => {
      const fleet =
        fleetContext.membershipId === null
          ...
          : await tx.fleet.findUnique({ where: { id: fleetContext.fleetId } });
      if (!fleet)
        throw new CodedException(404, 'RESOURCE_NOT_OWNED', 'Fleet not found.');
      return fleet;
    };
    const fleet = await this.fleetPath.run(actor, fleetContext, run, run);
    const owner = await this.system.user.findUnique({
  ...
  async currentTrip(
    ...
    if (!trip)
      throw new CodedException(
        404,
        'TRIP_ACCESS_DENIED',
        'No current trip on the assigned bus.',
      );
    return trip;
  }
  ...
  async tripById(
    ...
    const run = async (tx: Prisma.TransactionClient): Promise<Trip> =>
      (
        await this.assertAssignment(tx, {
          driverId: actor.id,
          fleetId: fleetContext.fleetId,
          tripId,
        })
  ...
  async manifest(
    ...
    const run = async (
      ...
      return bookings.map((b) => ({
        ...
        seatCount: b.seats,
        boardingStatus: b.boardedAt
          ? ('BOARDED' as const)
          : ('NOT_BOARDED' as const),
        dropOffStatus: (b.dropStatus ??
          'PENDING') as ManifestRow['dropOffStatus'],
      }));
    };
    return this.fleetPath.run(actor, fleetContext, run, run);
  }
  ...
  async board(
    ...
    const run = async (tx: Prisma.TransactionClient) => {
      ...
      if (row.boardedAt)
        return { boardingStatus: 'BOARDED', boardedAt: row.boardedAt };
      const now = new Date();
      ...
      if (updated.count === 0) {
        ...
        return {
          ...
          boardedAt: current.boardedAt as Date,
        };
      }
  ...
  async dropOff(
    ...
    if (input.status === 'DROPPED_OFF' && !input.stationId) {
      throw new CodedException(
        ...
        'stationId is required for DROPPED_OFF.',
        {
    ...
    if (input.status === 'NOT_DROPPED_OFF' && !input.reason) {
      throw new CodedException(
        409,
        'INVALID_DROPOFF_STATE',
        'reason is required for NOT_DROPPED_OFF.',
        {
    ...
    const run = async (tx: Prisma.TransactionClient) => {
      ...
      if (row.dropStatus) {
        if (row.dropStatus !== input.status) {
          throw new CodedException(
            409,
            'INVALID_DROPOFF_STATE',
            'Drop-off already recorded with a different status.',
          );
        }
      ...
      const updated = await tx.booking.updateMany({
        ...
        data: {
          ...
          dropStationId:
            input.status === 'DROPPED_OFF' ? (input.stationId as string) : null,
          dropReason:
            input.status === 'NOT_DROPPED_OFF'
      ...
      if (updated.count === 0) {
        ...
        return {
          dropStatus: current.dropStatus,
          droppedAt: current.droppedAt as Date,
        };
      }
      return { dropStatus: input.status, droppedAt: now };
    };
    const result = await this.fleetPath.run(actor, fleetContext, run, run);
    await this.auditDropOff(
      actor,
      fleetContext,
      tripId,
      bookingId,
    ...
    const run = async (tx: Prisma.TransactionClient) => {
      ...
      if (!row.boardedAt || row.status === 'CANCELLED') {
        throw new CodedException(
          409,
          'PAYMENT_NOT_ALLOWED',
          'Payment requires a boarded, un-cancelled booking.',
        );
      }
      if (row.paymentMethod && row.paymentMethod !== input.method) {
        throw new CodedException(
          409,
          'PAYMENT_NOT_ALLOWED',
          'Payment method does not match the booking.',
          {
      ...
      if (updated.count === 0) {
        ...
        return {
          paymentStatus: current.paymentStatus as string,
          paidAt: current.paidAt as Date,
        };
      }
