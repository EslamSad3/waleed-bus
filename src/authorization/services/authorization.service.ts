...
export class AuthorizationService {
  ...
  async fleetPermissions(userId: string, fleetId: string): Promise<Set<string>> {
    const keys = await this.tenantContext.withUserContext(userId, async (tx) => {
  ...
  async globalPermissions(userId: string): Promise<Set<string>> {
    const keys = await this.tenantContext.withUserContext(userId, async (tx) => {
      ...
      return assignments
        ...
        .flatMap((a) =>
          a.role.rolePermissions.filter((rp) => rp.permission.isActive).map((rp) => rp.permission.key),
        );
    });
    return new Set(keys);
  }
  ...
  async resolveFleetContext(userId: string, fleetId: string): Promise<FleetContext> {
    const context = await this.tenantContext.withUserContext(userId, async (tx) => {
      const membership = await tx.fleetMember.findUnique({
        where: { userId_fleetId: { userId, fleetId } },
        include: { role: true },
      });
      if (!membership || membership.status !== 'ACTIVE' || !membership.role.isActive) return null;
      return {
    ...
    if (!context) throw new ForbiddenException('No active membership in this fleet');
    return context;
  }
}
