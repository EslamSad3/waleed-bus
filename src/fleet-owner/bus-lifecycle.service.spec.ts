...
function makeService(db: {
  ...
  const tx = {
    bus: {
      ...
      update: vi.fn(async () => db.updated ?? { ...(db.bus as object), isActive: false }),
    },
    trip: {
  ...
  const fleetPath = {
    run: vi.fn(async (_actor: unknown, _fleet: unknown, tenantPath: (tx: unknown) => Promise<unknown>) =>
      tenantPath(tx),
    ),
  };
...
describe('BusLifecycleService', () => {
  it('disable blocks while a DEPARTED trip runs on the bus (409 BUS_ACTION_NOT_ALLOWED)', async () => {
    ...
    const error = await service.disable(ACTOR, FLEET, 'bus-1').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect((error as CodedException).getStatus()).toBe(409);
    expect(JSON.stringify((error as CodedException).getResponse())).toContain('BUS_ACTION_NOT_ALLOWED');
  });
  ...
  it('disable allows when only SCHEDULED trips exist (clarify Q3-A)', async () => {
    ...
    expect(tx.bus.update).toHaveBeenCalledWith({ where: { id: 'bus-1' }, data: { isActive: false } });
  ...
  it('disable rejects an already-inactive bus (409 BUS_ACTION_NOT_ALLOWED)', async () => {
    const { service, tx } = makeService({ bus: { id: 'bus-1', isActive: false } });
    const error = await service.disable(ACTOR, FLEET, 'bus-1').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect((error as CodedException).getStatus()).toBe(409);
  ...
  it('disable surfaces cross-fleet buses as 404 (no existence oracle)', async () => {
    ...
    const error = await service.disable(ACTOR, FLEET, 'foreign-bus').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
  ...
  it('reactivate flips an inactive owned bus back on', async () => {
    ...
    expect(tx.bus.update).toHaveBeenCalledWith({ where: { id: 'bus-1' }, data: { isActive: true } });
  ...
  it('reactivate rejects an already-active bus (409 BUS_ACTION_NOT_ALLOWED)', async () => {
    const { service, tx } = makeService({ bus: { id: 'bus-1', isActive: true } });
    const error = await service.reactivate(ACTOR, FLEET, 'bus-1').catch((e: unknown) => e);
    expect(error).toBeInstanceOf(CodedException);
    expect((error as CodedException).getStatus()).toBe(409);
    expect(tx.bus.update).not.toHaveBeenCalled();
  });
