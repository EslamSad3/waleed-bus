...
describe('OpenAPI document', () => {
  ...
  it('requires bearer auth everywhere except public endpoints', () => {
    ...
    for (const [path, method, op] of operations()) {
      ...
      if (publicOperations.has(key)) {
        ...
        expect(op.security, `${key} must require bearer auth`).toEqual([
          { bearer: [] },
        ]);
      }
    }
  ...
  it('documents cursor pagination query parameters on collection endpoints', () => {
    ...
    for (const path of collections) {
      const parameters = (doc.paths[path]?.get?.parameters ?? []) as Array<
        Record<string, any>
      >;
      const names = parameters.map((p) => p.name);
      expect(names, `${path} cursor/limit`).toEqual(
  ...
  it('documents uuid path parameters', () => {
    const busesList = (doc.paths['/fleets/{fleetId}/buses'].get?.parameters ??
    ...
    const busOne = (doc.paths['/fleets/{fleetId}/buses/{id}'].get?.parameters ??
      []) as Array<Record<string, any>>;
    ...
    const memberUpdate = (doc.paths['/fleets/{fleetId}/members/{memberId}']
      .patch?.parameters ?? []) as Array<Record<string, any>>;
  ...
  it('documents request DTO schemas with constraints, optional fields, and enums', () => {
    ...
    expect(createBus.properties.registrationNumber).toMatchObject({
      minLength: 1,
      maxLength: 50,
    });
    expect(createBus.required).toEqual(
      expect.arrayContaining(['registrationNumber', 'capacity']),
    );
    ...
    expect(createUser.properties.password).toMatchObject({
      minLength: 8,
      maxLength: 128,
    });
    ...
    expect(createRole.properties.slug).toMatchObject({
    ...
    expect(createTrip.properties.departAt).toMatchObject({
      format: 'date-time',
    });
    ...
    expect(createBooking.properties.status?.enum).toEqual([
      'CONFIRMED',
      'CANCELLED',
    ]);
    ...
    expect(addMember.properties.status?.enum).toEqual([
      'ACTIVE',
      'SUSPENDED',
      'REVOKED',
    ]);
  ...
  it('includes entity response schemas and never exposes password hashes', () => {
    ...
    expect(schemas.UserDto.properties).toMatchObject({
      email: { format: 'email' },
    });
    ...
    expect(schemas.FleetMemberDto.properties.status?.enum).toEqual([
      'ACTIVE',
      'SUSPENDED',
      'REVOKED',
    ]);
