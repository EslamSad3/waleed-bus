...
export function buildOpenApiDocument(app: INestApplication): OpenAPIObject {
  const config = new DocumentBuilder()
    ...
    .addTag(
      'auth',
      'Login, rotating refresh tokens, logout, and the verified session identity.',
    )
    .addTag('health', 'Liveness probe and service information.')
    .addTag(
      'roles',
      'Platform role administration — privileged path, super admin only, audited.',
    )
    .addTag(
      'permissions',
      'Platform permission catalog administration — privileged path, audited.',
    )
    .addTag('users', 'Platform user administration — privileged path, audited.')
    .addTag(
      'fleets',
      "Fleet administration (privileged) plus the caller's own memberships via `mine`.",
    )
    .addTag(
      'fleet-members',
      'Fleet membership management within a tenant fleet (tenant path).',
    )
    ...
    .addTag(
      'audit',
      'Platform audit log reads — privileged path, super admin only.',
    )
    .addTag(
      'passenger-auth',
      'Mobile passenger registration, OTP phone verification, and profile completion.',
    )
    .addTag(
      'passenger-trips',
      'Passenger trip search, station itineraries, and real-time seat inventory.',
    )
    .addTag(
      'passenger-bookings',
      'Passenger seat reservation, booking history, cancellation, and active trip tracking.',
    )
    .addTag('routes', 'Public QR route resolution and station itineraries.')
    .addTag(
      'public-shares',
      'Public read-only live tracking share verification.',
    )
    .build();
