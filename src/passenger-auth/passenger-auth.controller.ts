...
export class PassengerAuthController {
  ...
  @ApiOperation({
    summary:
      'Register a passenger with name + phone + password (pending verification).',
  })
  @ApiEnvelopeResponse(
    201,
    'Pending passenger; verification challenge opened for the phone.',
  )
  @ApiResponse({ status: 422, description: 'Invalid payload.' })
  @ApiResponse({
    status: 429,
    description: 'Send throttle exceeded (retryAfter seconds).',
  })
  ...
  @ApiResponse({
    status: 429,
    description: 'Resend cooldown or send throttle (retryAfter seconds).',
  })
  async sendOtp(@Body() dto: SendOtpDto) {
    const phoneNumber = normalizePhone(dto.phoneNumber);
    await this.checkBudget(`otp:send:${phoneNumber}`, SEND_BUDGET, 'send');
    const { expiresInSeconds } = await this.otp.openChallenge(
      phoneNumber,
      'PROFILE',
      null,
    );
  ...
  @ApiOperation({
    summary: 'Verify the OTP and mark the phone verified (single use).',
  })
  @ApiEnvelopeResponse(200, 'Phone verified.', ProfileStatusDto)
  @ApiResponse({
    status: 404,
    description: 'Invalid code (uniform, no oracle).',
  })
  @ApiResponse({
    status: 410,
    description: 'Expired challenge; request a new code.',
  })
  @ApiResponse({
    status: 429,
    description: 'Verify throttle exceeded (retryAfter seconds).',
  })
  ...
  @ApiAuthErrors()
  @ApiOperation({
    summary: 'Report profile completeness and phone verification state.',
  })
  @ApiEnvelopeResponse(
    200,
    'Completeness status with missing fields.',
    ProfileStatusDto,
  )
  ...
  @ApiOperation({
    summary:
      'Update name, phone, or picture. A new phone stays pending (60s OTP window) while the verified number keeps working.',
  })
  ...
  @ApiResponse({
    status: 409,
    description: 'Phone unavailable (non-revealing).',
  })
  @ApiResponse({
    status: 429,
    description:
      'Phone-change throttled: pending window still active or per-user budget exceeded (retryAfter seconds).',
  })
