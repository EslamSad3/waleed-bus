...
export class AuthController {
  ...
  @ApiOperation({
    summary:
      'Shared login: email platform login, PASSENGER phone / provider login, or FLEET_OWNER / DRIVER phone login.',
  })
  @ApiEnvelopeResponse(
    201,
  ...
  @ApiResponse({
    status: 401,
    description: 'Invalid credentials (uniform response).',
  })
  @ApiResponse({
    status: 403,
    description:
      'Correct credentials but phone verification required (PHONE_NOT_VERIFIED).',
  })
  login(
    ...
    return this.authService.loginPassengerPhone({
      phone: dto.phone as string,
      password: dto.password as string,
      ...context,
    });
  ...
  @ApiEnvelopeResponse(
    201,
    'Fresh token pair; the presented refresh token is revoked.',
    LoginResponseDto,
  )
  @ApiResponse({
    status: 401,
