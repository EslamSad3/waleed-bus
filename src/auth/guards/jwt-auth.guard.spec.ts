...
describe('JwtAuthGuard', () => {
  ...
  it('accepts a valid token and attaches the authenticated user', async () => {
    ...
    expect(tenant.withUserContext).toHaveBeenCalledWith(
      user.id,
      expect.any(Function),
    );
  });
  ...
  it('rejects when the stored authVersion has moved on', async () => {
    tenant.withUserContext.mockImplementation(
      async (_u: string, fn: (tx: unknown) => unknown) =>
        fn({
          user: {
            findUnique: vi.fn().mockResolvedValue({ ...user, authVersion: 3 }),
          },
          session: {
            findUnique: vi
              .fn()
  ...
  it('rejects an inactive user', async () => {
    tenant.withUserContext.mockImplementation(
      async (_u: string, fn: (tx: unknown) => unknown) =>
        fn({
          user: {
            findUnique: vi.fn().mockResolvedValue({ ...user, isActive: false }),
          },
          session: {
            findUnique: vi
              .fn()
  ...
  describe('passenger profile scope (spec 002)', () => {
    ...
    it('allows restricted sessions on @AllowRestricted() routes', async () => {
      ...
      const request: Record<string, unknown> = {
        headers: { authorization: `Bearer ${await passengerToken()}` },
      };
      const ctx = {
        switchToHttp: () => ({
          getRequest: () => request,
