...
export class LoginRequestDto {
  @ApiPropertyOptional({
    example: 'PASSENGER',
    enum: ['PASSENGER', 'FLEET_OWNER', 'DRIVER'],
  })
  @IsOptional()
  ...
  @ValidateIf(
    (o: LoginRequestDto) =>
      o.loginType === undefined || o.provider === undefined,
  )
  @IsString()
  @MinLength(8)
  @MaxLength(128)
  ...
  @ValidateIf(
    (o: LoginRequestDto) => o.loginType !== undefined && o.phone === undefined,
  )
  @IsIn(['GOOGLE', 'APPLE'])
  provider?: string;
...
export class RefreshDto {
  @ApiProperty({
    description:
      'The refresh token obtained from login/refresh. Single use — it rotates.',
...
export class LoginResponseDto {
  @ApiProperty({
    description:
      'HS256 access JWT carrying {sub, email, app_role, authVersion, sessionId}.',
  ...
  @ApiProperty({
    description:
      'Opaque rotating refresh token (7 days). Present it to /auth/refresh; never store it client-side in localStorage.',
    example: 'b3BhcXVlLXJlZnJlc2gtdG9rZW4',
  })
  refreshToken!: string;
}
...
export class CurrentUserDto {
  ...
  @ApiProperty({
    description:
      'Global role slug resolved from the database (super_admin priority, "user" fallback).',
    example: 'super_admin',
  })
  appRole!: string;
  ...
  @ApiProperty({
    description:
      'Bumped on security-sensitive changes; stale tokens are rejected.',
  })
  authVersion!: number;
