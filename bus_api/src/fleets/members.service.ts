...
export class MembersService {
    ...
    const membership = await this.withPath(
      ...
      () =>
        this.tenantContext.withFleetContext(
          { userId: actor.id, fleetId: fleetContext.fleetId },
          async (tx) => {
            const existing = await tx.fleetMember.findUnique({ where: { id: memberId } });
      ...
      (tx) =>
        this.updatePlatform(tx, fleetContext, memberId, input),
    );
  ...
  private withPath<T>(
    ...
    if (fleetContext.membershipId === null) {
      if (actor.appRole !== 'super_admin' || fleetContext.roleSlug !== 'super_admin') {
        throw new ForbiddenException('Platform context requires the super_admin role');
      }
  ...
  private async updatePlatform(
    ...
    const role = input.roleSlug ? await this.resolveRole(tx, { roleSlug: input.roleSlug }) : undefined;
    return tx.fleetMember.update({
      where: { id: memberId },
      data: { ...(role ? { roleId: role.id } : {}), ...(input.status ? { status: input.status } : {}) },
    });
  }

  private async removePlatform(tx: Prisma.TransactionClient, memberId: string): Promise<FleetMember> {
    const existing = await tx.fleetMember.findUnique({ where: { id: memberId } });
    if (!existing) throw new NotFoundException('Member not found');
    await tx.fleetMember.delete({ where: { id: memberId } });
    return existing;
